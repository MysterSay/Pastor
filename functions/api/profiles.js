...
export async function onRequestGet(context) {
  try {
    ...
    const NOTION_DATA_SOURCE_ID = context.env.NOTION_DATA_SOURCE_ID;

    if (!NOTION_TOKEN) {
      return json(
        {
          ok: false,
          error: "Не задано секрет NOTION_TOKEN у Cloudflare.",
        },
        500
      );
    }
    ...
    if (!NOTION_DATA_SOURCE_ID) {
      return json(
        {
          ok: false,
          error: "Не задано секрет NOTION_DATA_SOURCE_ID у Cloudflare.",
        },
        500
      );
    }
    ...
    return json(
      {
        ok: false,
        error: error?.message || "Невідома помилка при читанні з Notion.",
      },
      500
    );
  }
...
async function queryAllPages({ token, dataSourceId }) {
  ...
  while (hasMore) {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Notion API ${response.status}: ${notionMessage}`);
    }
